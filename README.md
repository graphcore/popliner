...
PopLiner relies on explicit layer names being present in the model - which must be added by the user.
The way this is done depends on the framework used, but generally it is achieved by using name scopes.
To ensure layer names have been correctly added, you may check to ensure:
  - For TensorFlow models, the `op_name` field in the `xla_op` debug context layers are populated.
  - For PopART models, the `opName` field in the `onnx` debug context layers are populated.

It is important that a particular naming format is used for the layers in the model.
Currently, PopLiner only recognises layer names that begin with "layer", "Layer" or "block".
